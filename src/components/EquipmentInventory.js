...
export class EquipmentInventory extends Component {
  ...
  async populateItems() {
    ...
    fetch(`${window.EQUIPMENTINVENTORY_ITEMS_API_URL}?userId=${userId}`, {
          ...
          <Table striped>
            ...
            <tbody>
              {!items || items.length <= 0 ?
                <tr>
                  <td colSpan="6" align="center"><b>No Equipment yet</b></td>
                </tr>
                : items.map(item => (
  ...
  render() {
    let contents = this.state.loading
      ...
      : this.state.loadedSuccess
        ...
        : <p>Could not load Equipment</p>;

    return (
      <div>
        <h1 id="tabelLabel" >{this.cameFromUsersPage() ? this.props.location.user.username : 'My'} EquipmentInventory</h1>
        {contents}
      </div>
    );
  }
